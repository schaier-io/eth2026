...
import { hasStoredDemoTermsAccepted } from "../components/DemoTermsGate";
...
function RegistryMarketsPanel({
  ...
  const reads = useReadContracts({
    contracts: list.flatMap((addr) => [
      ...
      { address: addr, abi: truthMarketAbi, functionName: "swarmReference" },
    ]),
  ...
  useEffect(() => {
    const pending = list
      .map((addr, i) => ({
        addr,
        reference: data?.[i * 3 + 2]?.result as Hex | undefined,
      }))
      .filter((entry) => entry.reference && !titles[entry.addr.toLowerCase()]);

    if (pending.length === 0) return;

    let cancelled = false;
    Promise.all(
      pending.map(async ({ addr, reference }) => {
        try {
          const res = await fetch(`/api/swarm/claim-doc?reference=${encodeURIComponent(reference!)}`);
          const body = (await res.json()) as { document?: { title?: string } | null; verified?: boolean };
          const title = body.verified && body.document?.title ? body.document.title : "(claim unavailable)";
          return [addr.toLowerCase(), title] as const;
        } catch {
          return [addr.toLowerCase(), "(claim unavailable)"] as const;
        }
      }),
    ).then((entries) => {
      if (cancelled) return;
      setTitles((prev) => {
        const next = { ...prev };
        for (const [market, title] of entries) next[market] = title;
        return next;
      });
    });
    ...
    <div className="registry-panel">
      ...
      {addressesLoading && list.length === 0 ? (
        ...
        <ul className="registry-list">
          {list.map((addr, i) => {
            // Cast through `any` — the union return type from useReadContracts
            // with a 50+ entry ABI overflows TypeScript's inference depth.
            const phase = data?.[i * 3]?.result as number | undefined;
            const outcome = data?.[i * 3 + 1]?.result as number | undefined;
            ...
            return (
              <li key={addr} className={`registry-row${active ? " is-active" : ""}`}>
                <button type="button" onClick={() => onSelect(addr)} className="registry-row-button">
                  <span className="registry-row-name">{titles[addr.toLowerCase()] ?? "Loading claim..."}</span>
...
export default function TruthMarketApp() {
  ...
  const contractReads = useReadContracts({
    contracts: activeMarketAddress
      ? [
          ...
          { address: activeMarketAddress, abi: truthMarketAbi, functionName: "swarmReference" },
        ]
      : [],
  ...
  const tokenAddress = readContractResult<Address | undefined>(19, undefined);
  ...
  const contractRiskPercent = Number(readContractResult(18, RISK_PERCENT));
  const selectedMarket = useMemo(() => {
    if (!activeMarketAddress || !contractReads.data) return selectedMarketBase;
    const phase = phaseFromContract(Number(readContractResult(0, 0)));
    const totalCommittedStake = readContractResult<bigint>(2, 0n);
    const distributablePool = readContractResult<bigint>(3, 0n);
    const randomness = readContractResult<bigint>(9, 0n);
    const randomnessHash = readContractResult<Hex>(10, ZERO_HASH);
    const randomnessIpfsAddress = readContractResult<Hex>(11, "0x");
    const randomnessSequence = readContractResult<bigint>(12, 0n);
    const randomnessTimestamp = readContractResult<bigint>(13, 0n);
    const randomnessIndex = readContractResult<number>(14, 0);
    const auditHash = readContractResult<Hex>(15, ZERO_HASH);
    return {
      ...selectedMarketBase,
      id: activeMarketAddress,
      title: selectedMarketBase.title,
      description: selectedMarketBase.description,
      phase,
      uiStage: phase,
      stake: tokenNumber(totalCommittedStake, tokenDecimals),
      commits: Number(readContractResult(1, selectedMarketBase.commits)),
      pool: tokenNumber(distributablePool, tokenDecimals),
      targetJurySize: Number(readContractResult(4, selectedMarketBase.targetJurySize)),
      minRevealedJurors: Number(readContractResult(5, selectedMarketBase.minRevealedJurors)),
      revealedJurors: Number(readContractResult(6, selectedMarketBase.revealedJurors)),
      juryUpCount: Number(readContractResult(7, selectedMarketBase.juryUpCount)),
      juryDownCount: Number(readContractResult(8, selectedMarketBase.juryDownCount)),
      ...
      jurors: readContractResult<Address[]>(16, []),
  ...
  function showScreen(nextScreen: Screen) {
    if (!hasStoredDemoTermsAccepted()) return;
    ...
    <div className="app-shell">
      <header className="topbar">
        <button className="brand" type="button" aria-label="Go to market feed" onClick={() => showScreen("feed")}>
          ...
          <img src="/brand-mark.svg" alt="" className="brand-mark" width={32} height={32} />
