...
export async function fetchLatestSpaceComputerBeacon(): Promise<LatestSpaceComputerBeacon> {
  ...
  if (!response.ok) {
    throw new Error(`SpaceComputer beacon fetch failed with HTTP ${response.status} ${response.statusText}`);
  }
...
export function parseSpaceComputerBeacon(rawBody: string): LatestSpaceComputerBeacon {
  ...
  try {
    ...
    throw new Error(`SpaceComputer beacon response is not valid JSON: ${e instanceof Error ? e.message : String(e)}`);
  }
  ...
  if (!Array.isArray(ctrng) || ctrng.length === 0) {
    throw new Error("SpaceComputer beacon data.ctrng must be a non-empty array");
  }
  ...
  if (randomness === 0n || randomness > MAX_UINT256) {
    throw new Error("SpaceComputer cTRNG value must be a non-zero uint256");
  }

  const sequence = parseUint64(data.sequence, "data.sequence");
  const timestamp = parseUint64(data.timestamp, "data.timestamp");
  if (timestamp === 0n) {
    throw new Error("SpaceComputer beacon data.timestamp must be non-zero");
  }
...
function parseCtrngHex(value: unknown): Hex {
  if (typeof value !== "string") {
    throw new Error("SpaceComputer cTRNG value must be a hex string");
  }
  const trimmed = value.trim();
  const normalized = trimmed.startsWith("0x") ? trimmed : `0x${trimmed}`;
  if (!isHex(normalized) || normalized.length <= 2 || normalized.length > 66) {
    throw new Error("SpaceComputer cTRNG value must fit uint256 hex");
  }
...
function parseUint64(value: unknown, label: string): bigint {
  ...
  if (parsed < 0n || parsed > MAX_UINT64) {
    throw new Error(`${label} is outside uint64 range`);
  }
  return parsed;
}
