...
- jury draw size is dynamic: the largest odd value no greater than `min(maxJurors, max(minJurors, activeCommitters * 15 / 100))`, so small markets draw the minimum juror floor and larger markets scale up to the configured maximum without even-sized juries;
